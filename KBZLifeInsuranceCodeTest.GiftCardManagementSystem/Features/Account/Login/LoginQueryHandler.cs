...
namespace KBZLifeInsuranceCodeTest.GiftCardManagementSystem.Features.Account.Login
{
    public class LoginQueryHandler : IRequestHandler<LoginQuery, Result<JwtResponseModel>>
    {
        ...
        public LoginQueryHandler(IAccountRepository accountRepository, LoginValidator loginValidator, JwtService jwtService)
        {
        ...
        public async Task<Result<JwtResponseModel>> Handle(LoginQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var validationResult = await _loginValidator.ValidateAsync(request.LoginRequest, cancellationToken);
