...
namespace KBZLifeInsuranceCodeTest.Shared
{
    public static class DevCode
    {
        ...
        public static string GeneratePromoCode()
        {
            ...
            string digits = new string(Enumerable.Range(0, 6).Select(x => random.Next(0, 10).ToString()[0]).ToArray());

            // Generate 5 random uppercase letters
            string alphabets = new string(Enumerable.Range(0, 5).Select(x => (char)random.Next('A', 'Z' + 1)).ToArray());
